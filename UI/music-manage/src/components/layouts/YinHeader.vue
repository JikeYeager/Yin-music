...
export default defineComponent({
  ...
  setup() {
    ...
    // const username = ref("admin");  //这里读取固定值admin，需要我们去修改
    const username = sessionStorage.getItem("username"); //从缓存中读取username的值才能在前端中改变对应右上角的用户名
    const userPic = computed(() => store.getters.userPic); //从Vuex（内存存储）中取出图片
