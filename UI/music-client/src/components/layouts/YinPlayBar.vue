  ...
  <div class="play-bar" :class="{ show: !toggle }">
    ...
    <div class="control-box">
      ...
      <div @click="goPlayerPage">
         <el-image class="song-bar-img" fit="contain" :src="attachImageUrl(this.songPic)"/>
      </div>
        <!--播放开始结束时间-->
        <div v-if="songId">
...
export default defineComponent({
  ...
  setup() {
    ...
    const isCollection = ref(false); // 是否收藏  isCollection类型可知是bool类型
    ...
    //通过事件监听，实现点一次改变一次isCollection的值
    watch(token, (value) => {
    ...
    async function changeCollection() {
      ...
      //1. 根据isCollection的值来判断是进行设置收藏还是删除收藏的请求
      ...
      //这里要求后端有返回值，然后根据返回值修改isCollection的值，
      // 进而实现点击一次改变一次“收藏”的图标
